...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "You are an impartial evaluator. Score the answer on every rubric "
            "criterion from 0.0 to 1.0. Return only a JSON object mapping each "
        ...
        try:
            ...
            candidate_scores = (
                parsed.get("scores", parsed) if isinstance(parsed, dict) else {}
            )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if item_averages:
            overall_average = sum(item_averages) / len(item_averages)
        else:
            overall_average = 0.5

        # Comparative experiments are represented as adjacent pairs:
        # [first-position result, second-position result, ...]. A single result
        # cannot provide evidence of position bias.
        pairs = list(zip(item_averages[0::2], item_averages[1::2]))
        ...
        return {
            ...
            "leniency_bias": bool(item_averages) and overall_average > 0.8,
            "severity_bias": bool(item_averages) and overall_average < 0.3,
        }
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            suggestion = (
                ...
                else "Review and define a corrective action"
            )
            rows.append(
                f"| F{index:03d} | {escape_cell(failure.failure_type or 'unknown')} "
    ...
    def generate_improvement_suggestions(
        ...
        action_map = {
            "hallucination": "Implement claim grounding and citation checks to filter unsupported statements",
            "irrelevant": "Add intent-routing tests and few-shot examples so answers address the question directly",
            "incomplete": "Tune retrieval top-k and prompt the generator to cover every required condition",
            "off_topic": "Improve intent detection and add an out-of-scope response policy",
            "refusal": "Review guardrail rules and add examples of safe questions that should be answered",
            "unknown": "Manually review uncategorized failures and assign a consistent failure type",
        }
        ...
        suggestions = [
            action_map.get(
                failure_type.lower(),
                f"Review {failure_type} failures and implement a targeted regression test",
            )
            for failure_type in prioritized_types
        ]

        general_actions = [
            "Add representative failures to the golden dataset as regression cases",
            "Review low-scoring cases with human annotators and calibrate evaluation thresholds",
            "Track answer and retrieval metrics separately after every prompt, model, or corpus change",
        ]
